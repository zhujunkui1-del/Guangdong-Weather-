# -*- coding: utf-8 -*-
"""
Google Colab APK Builder for Guangdong Weather
Copy-paste this entire script into a Google Colab cell and run it.
It will output a GuangdongWeather.apk file for download.
"""
import os, subprocess, shutil

WORK = "/content/weather_build"
...
T = {
    "en": {
        "title": "Guangdong Weather", "city_frame": "City",
        "auto_cb": "Auto-refresh (60s)", "refresh_btn": "Refresh Now",
        "fetching": "Fetching...", "ready": "Ready",
        "updated": "Last update: ", "next": "Next: ",
        "refreshing": "Refreshing...", "loading": "Loading...",
        "lang_btn": "\u4e2d\u6587", "location_label": "Location",
        "temp": "Temp", "feels_like": "FeelsLike", "humidity": "Humidity",
        "wind": "Wind", "visibility": "Visibility", "pressure": "Pressure",
        "weather": "Weather", "forecast_title": "3-Day Forecast",
        "avg": "avg", "sunrise": "Sunrise", "sunset": "Sunset",
        "error_network": "Network error", "sep": "=" * 50,
    },
    "zh": {
        "title": "\u5e7f\u4e1c\u7701\u5929\u6c14\u5b9e\u65f6\u67e5\u8be2",
        "auto_cb": "\u81ea\u52a8\u5237\u65b0(60\u79d2)",
        "refresh_btn": "\u7acb\u5373\u5237\u65b0",
        "fetching": "\u83b7\u53d6\u4e2d...", "ready": "\u5c31\u7eea",
        "updated": "\u6700\u540e\u66f4\u65b0: ", "next": "\u4e0b\u6b21: ",
        "refreshing": "\u5237\u65b0\u4e2d...", "loading": "\u52a0\u8f7d\u4e2d...",
        "lang_btn": "English", "location_label": "\u5730\u70b9",
        "temp": "\u6e29\u5ea6", "feels_like": "\u4f53\u611f",
        "humidity": "\u6e7f\u5ea6", "wind": "\u98ce\u529b",
        "visibility": "\u80fd\u89c1\u5ea6", "pressure": "\u6c14\u538b",
        "weather": "\u5929\u6c14", "forecast_title": "\u672a\u6765\u4e09\u65e5\u9884\u62a5",
        "avg": "\u5747\u6e29", "sunrise": "\u65e5\u51fa", "sunset": "\u65e5\u843d",
        "error_network": "\u7f51\u7edc\u9519\u8bef", "sep": "=" * 50,
    }
}
...
class WeatherRoot(BoxLayout):
    def __init__(self, **kwargs):
        ...
        t = T[self.lang]
        self.title_label = Label(text=t["title"], font_size="18sp", bold=True, size_hint_y=None, height=40)
        self.add_widget(self.title_label)
        self.lang_btn = Button(text=t["lang_btn"], size_hint=(None, None), size=(100, 36), pos_hint={"right": 1})
        self.lang_btn.bind(on_press=self._switch_lang)
        self.add_widget(self.lang_btn)
        cl = GridLayout(cols=4, size_hint_y=None, height=44, spacing=6)
        for name, q in [("Guangdong","Guangdong"),("Zhanjiang","Zhanjiang"),("Guangzhou","Guangzhou"),("Jieyang","Jieyang")]:
        ...
        self.auto_label = Label(text=t["auto_cb"], font_size="12sp", size_hint_x=None, width=180, halign="left")
        ctrl.add_widget(self.auto_label)
        self.cd_label = Label(text="OFF", font_size="11sp", size_hint_x=None, width=120, color=(0.6, 0.6, 0.6, 1))
        ctrl.add_widget(self.cd_label)
        self.refresh_btn = Button(text=t["refresh_btn"], font_size="12sp", size_hint_x=1)
        self.refresh_btn.bind(on_press=lambda x: self.refresh())
        ctrl.add_widget(self.refresh_btn)
        self.add_widget(ctrl)
        self.sv = ScrollView()
        self.output = TextInput(text=t["loading"], readonly=True, background_color=(0.12, 0.12, 0.18, 1), foreground_color=(0.8, 0.84, 0.96, 1), font_name="Data/Fonts/DroidSansMono.ttf")
        self.sv.add_widget(self.output)
        self.add_widget(self.sv)
        self.status = Label(text=t["ready"], font_size="11sp", size_hint_y=None, height=28, color=(0.5, 0.5, 0.5, 1))
    ...
    def refresh(self):
        ...
        self.busy = True
        t = T[self.lang]
        self.refresh_btn.text = t["fetching"]; self.refresh_btn.disabled = True
        self.status.text = t["fetching"]; self.output.text = t["fetching"]
    ...
    def _show(self, text, ts):
        ...
        self.refresh_btn.text = t["refresh_btn"]; self.refresh_btn.disabled = False
    ...
    def _toggle_auto(self, cb, value):
        ...
        t = T[self.lang]
        if value:
            self.cd_label.text = f"{t['next']}60s"; self.cd_label.color = (0.25, 0.63, 0.17, 1)
            self._schedule(60)
        else:
            if self.timer_event: self.timer_event.cancel(); self.timer_event = None
            self.cd_label.text = "OFF"; self.cd_label.color = (0.6, 0.6, 0.6, 1)

    def _schedule(self, n):
        t = T[self.lang]
        if n <= 0:
            self.cd_label.text = t["refreshing"]; self.cd_label.color = (1, 0.39, 0.04, 1)
            self.refresh()
            self.timer_event = Clock.schedule_once(lambda dt: self._schedule(60), 0.5)
        else:
            self.cd_label.text = f"{t['next']}{n}s"; self.cd_label.color = (0.25, 0.63, 0.17, 1)
...
# ============ Build APK ============
print("\nBuilding APK (this takes 5-15 minutes)...")
result = subprocess.run(["buildozer", "-v", "android", "debug"], capture_output=False)
if result.returncode == 0:
    # Copy APK to a known location
    apk_path = "/content/weather_build/bin/weather-1.0-arm64-v8a-debug.apk"
    if os.path.exists(apk_path):
        shutil.copy(apk_path, "/content/GuangdongWeather.apk")
        print(f"\nAPK built successfully: /content/GuangdongWeather.apk")
    else:
        import glob
        apks = glob.glob("bin/*.apk")
        if apks:
            shutil.copy(apks[0], "/content/GuangdongWeather.apk")
            print(f"\nAPK built: /content/GuangdongWeather.apk")
        else:
            print("\nAPK not found in bin/ directory")
else:
